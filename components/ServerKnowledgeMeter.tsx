...
const ServerKnowledgeMeter: React.FC<ServerKnowledgeMeterProps> = ({ level, onClick, themeStyles }) => {
  const safeLevel = Math.max(0, Math.min(100, level));

  return (
    <button 
        onClick={onClick}
        className={`w-full text-left p-2 rounded-lg transition-colors ${themeStyles.interactive_panel}`}
        title="Click to view Learning Analytics"
    >
      <div className="flex justify-between items-center mb-1">
        <span className={`text-sm font-semibold ${themeStyles.textSecondary}`}>Server Knowledge Level</span>
